...
def checkDocument(text, key=None):
    ...
    service = h.request("https://service.afterthedeadline.com",method="POST",headers={'Content-type': 'application/x-www-form-urlencoded'}, body=params)[1]
    print(service)
    # service.request("POST", "/checkDocument", params)
    # response = service.getresponse()
    # if response.status != httplib2.:
    #     service.close()
...
def stats(data, key=None):
    ...
    service = h.request("https://service.afterthedeadline.com")
